...
def configurations_from_contract(contract: dict[str, Any]) -> list[UncertaintyConfiguration]:
    ...
    if len(configurations) != expected:
        raise UncertaintyCalibrationError(
            f"Expected {expected} configurations, observed {len(configurations)}"
        )
...
def weighted_interval_score(
    ...
    for coverage in sorted(intervals):
        ...
        weighted = weighted + (alpha / 2.0) * interval_score(
            y,
            lower,
            upper,
            alpha,
        )
    return weighted / (len(intervals) + 0.5)
    ...
    for fold_id, fold_frame in point_predictions.groupby("fold_id", sort=True):
        ...
        for row in fold.itertuples(index=False):
            ...
            record: dict[str, Any] = {
                ...
                "prediction_origin": pd.Timestamp(row.prediction_origin),
                "actual": actual,
...
def summarize_configuration(
    ...
    *,
    ...
    for narrower, wider in zip(ordered_levels[:-1], ordered_levels[1:]):
        ...
        nested &= (
            predictions[f"lower_{wide_suffix}"].to_numpy(dtype=float)
            <= predictions[f"lower_{narrow_suffix}"].to_numpy(dtype=float)
        ) & (
            predictions[f"upper_{wide_suffix}"].to_numpy(dtype=float)
            >= predictions[f"upper_{narrow_suffix}"].to_numpy(dtype=float)
        )
    ...
    primary_coverage_rows = coverage_frame.loc[
        coverage_frame["coverage_level"].eq(primary_coverage)
    ]
