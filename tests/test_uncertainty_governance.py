...
def test_gate_6e1_contract_and_closure_schemas_are_valid() -> None:
    contract_schema = _load_json(SCHEMAS / "uncertainty_contract.schema.json")
    closure_schema = _load_json(SCHEMAS / "gate_6e1_closure_manifest.schema.json")
...
def test_gate_6e1_freezes_method_and_configuration_set() -> None:
    ...
    assert contract["optimization"]["reference_configuration"] == REFERENCE_CONFIGURATION
...
def test_gate_6e1_preserves_point_model_and_locked_test_boundaries() -> None:
    ...
    assert contract["point_model_boundary"]["model_id"] == "v1_frozen_champion"
    assert contract["point_model_boundary"]["selected_model"] == "hist_gradient_boosting"
    assert contract["point_model_boundary"]["point_model_search_permitted"] is False
    assert contract["point_model_boundary"]["point_prediction_mutation_permitted"] is False
    assert contract["evidence_boundary"]["locked_test_partition_permitted"] is False
    assert contract["evidence_boundary"]["locked_prediction_parsing_permitted"] is False
    assert contract["evidence_boundary"]["confirmatory_evaluation_permitted"] is False
